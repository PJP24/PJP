...
async def add_user(username: str, email: str, password: str):
    ...
    response_data = await fetch_data("http://fastapi_orchestrator:5001/add_user", method="POST", data=user_data)
...
async def update_user_password(user_id: int, old_password: str, new_password: str):
    ...
    response_data = await fetch_data(f"http://fastapi_orchestrator:5001/update_password/{user_id}", method="PATCH", data=passwords)
...
async def delete_user(user_id: int):
    ...
    response_data = await fetch_data(f"http://fastapi_orchestrator:5001/delete_user/{user_id}", method="DELETE")
...
async def add_subscription_resolver(email: str, subscription_type: str):
    ...
    result_info = await fetch_data("http://fastapi_orchestrator:5001/add_subscriptions", method="POST", data=subscription_data)
...
async def change_subscription_resolver(email: str, subscription_type: str):
    ...
    subscription_data = {"email": email, "subscription_type": subscription_type}
    result_info = await fetch_data("http://fastapi_orchestrator:5001/change_subscriptions", method="PUT", data=subscription_data)
...
async def delete_subscription_resolver(email: str):
    ...
    result_info = await fetch_data(f"http://fastapi_orchestrator:5001/subscriptions/{email}", method="DELETE")
...
async def activate_subscription_resolver(email: str):
    ...
    result_info = await fetch_data(f"http://fastapi_orchestrator:5001/subscriptions/{email}/activate", method="POST")
...
async def deactivate_subscription_resolver(email: str):
    ...
    result_info = await fetch_data(f"http://fastapi_orchestrator:5001/subscriptions/{email}/deactivate", method="POST")
