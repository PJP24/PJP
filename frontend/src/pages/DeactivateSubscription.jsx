...
import { DEACTIVATE_SUBSCRIPTION } from '../GraphQL/Mutations';

const DeactivateSubscription = () => {
    const [email, setEmail] = useState("");
    const [result, setResult] = useState(null);

    const [deactivateSubscription, { error, loading }] = useMutation(DEACTIVATE_SUBSCRIPTION);
    ...
    const handleSubmit = async (e) => {
        ...
        if (response.data) {
            setResult(response.data.deactivateSubscription.resultInfo);
        }
    };

    return (
        <div className="max-w-md mx-auto bg-white shadow-md rounded-lg p-6 mt-10">
            <h2 className="text-2xl font-bold text-center mb-4">Deactivate Subscription</h2>
