...
from orchestrator_monolith.src.orchestrator.workflows.delete_user_workflow import (
    DSLInput,
    DeleteUserWorkflow,
)
...
class Orchestrator:
    ...
    async def activate_subscription(self, email: str, amount: float):
        ...
        if amount != required_amount:
            email_result = await self.send_unsuccessful_payment_email(
        ...
        try:
            async with grpc.aio.insecure_channel(self.subscription_host) as channel:
                ...
                response = await stub.ActivateSubscription(
                    ActivateSubscriptionRequest(user_id=user_id)
                )
                status = getattr(response, "status", "unknown")
                message = getattr(response, "message", "No message provided")
                email_result = await self.send_successful_payment_email(
                    email=email, username=username
                )
                return {
                    "status": status,
            ...
            return {
                "status": "error",
                "message": f"Error activating subscription: {e} | {email_result['message']}",
            }
    ...
    # async def load_yaml(self, filepath: str):
    ...
    async def delete_user(self, user_id: int):
        ...
        try:
            with open("orchestrator_monolith/src/orchestrator/workflows/delete-user.yaml", "r") as yaml_file:
                dsl_yaml = yaml_file.read()
                dsl_yaml = dsl_yaml.replace("${user_id}", str(user_id))
            ...
            result = await client.execute_workflow(
                ...
                task_queue="user-service-task-queue",
            )
            return result
        except Exception as e:
            return {"error": f"Error deleting user: {str(e)}"}
    ...
    async def send_subscription_expiration_notification(
        self, email: str, end_date: str
    ):
        try:
            notification_data = {
                "email": email,
            ...
            await self.send_to_kafka(
                topic="expiring_subscriptions", message=notification_data
            )
    ...
    async def send_emails_for_expiring_subscriptions(self):
        try:
            ...
            async with grpc.aio.insecure_channel(self.subscription_host) as channel:
                ...
                logger.info(
                    "Fetching expiring subscriptions from subscription service..."
                )
            ...
            for sub in response.subscriptions:
                ...
                email = user["email"]
                logger.info(
                    f"Sending email to {email} about subscription expiring on {sub.end_date}"
                )

                notification_result = (
                    await self.send_subscription_expiration_notification(
            ...
            return {
                "status": "success",
                "message": "Emails sent for expiring subscriptions.",
            }
            ...
            return {
                "status": "error",
                "message": f"Error sending expiring subscription notifications: {e}",
            }
